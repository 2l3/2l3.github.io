...
{
    [Authorize(Roles = "HospitalDeviceScheduleNotification")]
    public class DeviceScheduleNotificationController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            return View(db.DeviceSchedule.AsNoTracking().Where(s => s.IsDeleted == false && CurrentDay >= s.DayId && CurrentMonth == s.MonthId && Year > s.LastCloseYear).Include(s => s.Device).Include(s => s.Device.DeviceUnit).OrderByDescending(s => s.Device.Id));
        }
        ...
        public ActionResult CloseNotification(string Ids)
        {
            ...
            foreach(var i in arr)
            {
                ...
                var D = db.Device.Find(S.DeviceId);
